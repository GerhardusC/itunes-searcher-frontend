...
const SearchResultsDisplay = () => {
    ...
    if(searchResults && searchResults.resultCount > 0){
        ...
        return (
            <div>
                <p>Displaying {searchResults.resultCount} results for <b>{searchTerm}</b>.</p>
                        ...
                        <tbody>
                            {
                                searchIds.map((key) => {
                                    return(
                                        <tr key={key} className={!currentIds.includes(searchResults.results[key].trackViewUrl) && !currentIds.includes(searchResults.results[key].collectionViewUrl) ? "not-favourited-row" : "already-favourited-row"}>
                                            {/* 
                                                Depending on which artwork exists, we display it in the table, some results only have 30px and some have only 60 px etc.
                                            */}
                                            <td><img src={searchResults.results[key].artworkUrl30 || searchResults.results[key].artworkUrl60 || searchResults.results[key].artworkUrl100} alt="art"/></td>
                                            {/* 
                                            ...
                                            */}
                                            <td>{searchResults.results[key]?.trackName || searchResults.results[key].collectionName }</td>
                                            ...
                                            <td>
                                                ...
                                                */}
                                                {!currentIds.includes(searchResults.results[key].trackViewUrl) && !currentIds.includes(searchResults.results[key].collectionViewUrl) ? 
                                                    (<button className="favourite-button" onClick={() => dispatch(addFavourite(searchResults.results[key]))}>⭐</button>) : 
                                                    (<></>)}
                                            </td>
                                        </tr>
                                    )
    ...
    if(searchResults.resultCount === 0 && searchResults.results.wrapperType === "error"){
        return (
            <>
    ...
    if(searchResults.resultCount === 0){
        return(
            <div>No results found for <b>{searchTerm}</b>.</div>
        )
    }
}
