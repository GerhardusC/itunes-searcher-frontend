...
const favouritesSlice = createSlice({
    ...
    reducers: {
        ...
        addFavourite: (state, action) => {
            if (!state.currentIds.includes(action.payload.trackViewUrl) && !state.currentIds.includes(action.payload.collectionViewUrl)){
        ...
        removeFavourite: (state, action) => {
            for(let i = 0; i < state.data.length; i++){
                if(action.payload.wrapperType === "track" && state.data[i].trackViewUrl === action.payload.trackViewUrl){
                    state.data.splice(i, 1)
                    state.currentIds.splice(state.currentIds.indexOf(action.payload.trackViewUrl), 1)
                    return;
                }
                if(action.payload.wrapperType === "audiobook" && state.data[i].collectionViewUrl === action.payload.collectionViewUrl){
                    state.data.splice(i, 1)
                    state.currentIds.splice(state.currentIds.indexOf(action.payload.collectionViewUrl), 1)
                    return;
                }
            }
        } 
